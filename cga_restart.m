...
cc= numel(Cost_val);
current_iteration= cc-1;

current_cost= Cost_val{1,cc};

current_population= Coords{1,cc};

current_functional_evaluation= k(1,cc);

current_elapsed_time= time(1,cc);

current_max_cost= maxc(1,cc);

current_mesh= mesh{1,cc};

patch('Vertices',[current_mesh.x,current_mesh.y],'Faces',current_mesh.tri,...
...
[current_cost,ind]=sort(current_cost,'descend'); 

current_population=current_population(ind,:); %sorting population based on sorted cost values
  

[current_max_cost,imax] = max(current_cost);
  
    
  for i=1:popsize;
    plot(current_population(i,1:2:end),current_population(i,2:2:end),'r+')
  end;
  
plot(current_population(imax,1:2:end),current_population(imax,2:2:end),'kd',...
...
iga=current_iteration; %generation counter initialized

% Randomly generating initial population
par= current_population;



 
count=current_functional_evaluation;
...
time(iga+1)= toc+current_elapsed_time;
...
final_time= toc+current_elapsed_time;
disp(['Total Elapsed Time=' num2str(final_time)])
